    ...
    protected void paintGrawLine(Graphics g, int x1, int y1, int x2, int y2, Color saberColor) {
        ...
        if (mainColor == null) { // 描画処理の軽量化
            ...
            for (int i = 8; i >= 1; i--) {
                ...
                ousStrokes.add(new BasicStroke(12f + i * 2f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));

                ousColors.add(new Color(saberColor.getRed(), saberColor.getGreen(), saberColor.getBlue(), (int) (255 * alpha)));
            }
            ...
            for (int i = 3; i >= 1; i--) {
                ...
                mergeStrokes.add(new BasicStroke(6f + i * 1.2f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND));
                mergeColors.add(new Color(r, gC, b, 160));
            }

            // 中心の白い芯（太め）
            coreStroke = new BasicStroke(5.5f, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND);
